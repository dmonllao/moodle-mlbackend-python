...
def training():
    ...
    binary_classifier = estimator.Binary(uniquemodelid, outputdir)
    result = binary_classifier.train_dataset(datasetpath)
...
def prediction():
    ...
    binary_classifier = estimator.Binary(uniquemodelid, outputdir)
    result = binary_classifier.predict_dataset(datasetpath)
...
def evaluation():
    ...
    binary_classifier = estimator.Binary(uniquemodelid, outputdir)
    result = binary_classifier.evaluate_dataset(datasetpath,
                                                float(minscore),
                                                float(maxdeviation),
                                                int(niterations),
                                                trainedmodeldir)
...
def export():
    ...
    binary_classifier = estimator.Binary(uniquemodelid, modeldir)
    exportdir = binary_classifier.export_classifier(tempdir)
...
def import_model():
    ...
    with zipfile.ZipFile(importzippath, 'r') as zipobject:
        ...
        binary_classifier = estimator.Binary(uniquemodelid, modeldir)
        binary_classifier.import_classifier(importtempdir)
